...
class ProcessesScreen extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<ProcessesScreen> createState() => _State();
}

class _State extends ConsumerState<ProcessesScreen> {
  ...
  Future<void> _fetch() async {
    try {
      final r = await _dio!.get<dynamic>('/api/system/processes', queryParameters: {'sort': _sort, 'limit': 50});
      if (mounted) setState(() { _procs = (r.data as List).cast<Map<String,dynamic>>(); _loading = false; _error = null; });
        ...
        appBar: AppBar(
          ...
          actions: [
            DropdownButton<String>(
              ...
              items: const [
                DropdownMenuItem(value: 'cpu', child: Text('CPU', style: TextStyle(color: Colors.white))),
                DropdownMenuItem(value: 'mem', child: Text('MEM', style: TextStyle(color: Colors.white))),
              ],
                      ...
                      itemBuilder: (_, i) {
                        ...
                        return InkWell(
                          ...
                          child: Padding(
                            ...
                            child: Row(children: [
                              SizedBox(width: 50, child: Text('${p['pid']}', style: const TextStyle(color: Color(0xFF555555), fontSize: 12, fontFamily: 'monospace'))),
                              Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                                Text(p['name'] as String, style: const TextStyle(fontSize: 13), overflow: TextOverflow.ellipsis),
                                if ((p['username'] as String?)?.isNotEmpty == true)
                                  Text(p['username'] as String, style: const TextStyle(color: Color(0xFF555555), fontSize: 11)),
                              ])),
                              SizedBox(width: 50, child: Text(cpu.toStringAsFixed(1),
                                style: TextStyle(color: cpu > 50 ? const Color(0xFFFF5370) : cpu > 20 ? const Color(0xFFFFCB6B) : const Color(0xFF888888), fontSize: 12, fontFamily: 'monospace'),
                                textAlign: TextAlign.right)),
                              SizedBox(width: 50, child: Text(mem.toStringAsFixed(1),
                                style: TextStyle(color: mem > 20 ? const Color(0xFFFF5370) : mem > 10 ? const Color(0xFFFFCB6B) : const Color(0xFF888888), fontSize: 12, fontFamily: 'monospace'),
                                textAlign: TextAlign.right)),
                            ]),
                          ),
                        );
  ...
  void _killDialog(Map<String,dynamic> p) {
    showDialog(context: context, builder: (ctx) => AlertDialog(
      ...
      actions: [
        ...
        TextButton(
          onPressed: () async { Navigator.pop(ctx); await _dio!.post<dynamic>('/api/system/processes/${p['pid']}/kill'); await _fetch(); },
